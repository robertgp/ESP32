...
// UART definitions
#define UART_TX_GPIO_NUM (GPIO_NUM_4) // A0
#define UART_RX_GPIO_NUM 23 // SDA
...
static void uart_init() {
  ...
  uart_config_t uart_config = {
      .baud_rate = 1200, // Slow BAUD rate //2400
    ...
    if (len_in >0) {
      for (int i=0; i < 24; i++) {
        if (data_in[i] == 0x0A) {
          ...
          printf("Received comm from ID 0x%02X with code, 0x%02X \n", rxID, rcode);

          //PAYLOAD created HERE
          snprintf(data_received, sizeof(data_received), "fob=%d&hub=0&code=%d ", rxID, rcode);
...
static const char *payload = data_received;
